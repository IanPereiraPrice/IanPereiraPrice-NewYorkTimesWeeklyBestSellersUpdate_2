...
dag = DAG(dag_id="simple_etl_dag", default_args=args, schedule_interval='@daily')
...
def nyt_call(table_name,api_call_name,mail_list = []):
    ...
    if max(df['time'])>max(df['time']): 
    
        df.to_sql(table_name, engine, if_exists='replace')
        send_mail(["ianprice16@yahoo.com"])
